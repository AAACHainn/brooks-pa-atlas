...
- `备份`：导出一个 zip 文件，包含全部树状索引、图片文件、标题、备注、OCR 文本、OCR 状态，以及完整考试数据。
- `恢复`：导入备份 zip。恢复采用合并覆盖模式，相同 SHA-256 hash 的图片会覆盖元数据和所属索引，不会重复生成一份，也不会删除当前系统中备份外的数据；试卷题目会按图片 hash 重新映射到当前图库图片。
