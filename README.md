...
- `备份`：导出一个 zip 文件，包含全部树状索引、图片文件、标题、备注、标签、OCR 文本、OCR 状态，以及完整考试数据。
- `恢复`：导入备份 zip。恢复采用合并覆盖模式，相同 SHA-256 hash 的图片会覆盖元数据、标签和所属索引，不会重复生成一份，也不会删除当前系统中备份外的数据；试卷题目会按图片 hash 重新映射到当前图库图片。恢复旧版 v1 / v2 备份时会保留当前系统中已有图片的标签。
...
- 点击左侧索引可以筛选图片。
- 顶部搜索框可以搜索标题、OCR 文本、备注、索引路径和标签。
- 浏览模式标题栏和图片卡片会显示标签；点击标签可以叠加精确标签筛选。
